...
var LANDING_SOCIAL_PROOF_SPOTLIGHT = [
  {
    logo: '/assets/images/logo/logo-2-color.png',
    name: "Rizo's Pizzeria",
    quote: 'Real-time order tracking for customers and a kitchen dashboard that keeps every order moving from pending to done.',
    meta: 'Restaurant · online ordering'
  },
  {
    logo: '/assets/images/logo/logo-1-color.png',
    name: 'HOA communities',
    quote: 'Portals and tools built for how neighborhoods and boards actually run day to day.',
    meta: 'HOA · management portal'
  },
  {
    logo: '/assets/images/logo/logo-3-color.png',
    name: 'ProCleaning Seattle',
    quote: 'Field service workflows that keep crews, schedules, and customers in sync from the job site.',
    meta: 'Cleaning · field service'
  },
  {
    logo: '/assets/images/logo/logo-4-color.png',
    name: 'Local service brands',
    quote: 'Websites and booking flows that turn searches into scheduled work — without the back-and-forth.',
    meta: 'Trades · booking & CRM'
  },
  {
    logo: '/assets/images/logo/logo-5-color.png',
    name: 'Real estate teams',
    quote: 'Listings, lead capture, and client tools in one place instead of scattered spreadsheets.',
    meta: 'Real estate · lead portal'
  }
];

var landingSpotlightTimers = new WeakMap();

function escapeLandingSpotlightText(value) {
  return String(value == null ? '' : value)
    .replace(/&/g, '&amp;')
    .replace(/</g, '&lt;')
    .replace(/>/g, '&gt;')
...
function stopLandingSocialProofSpotlight(root) {
  if (!root) return;
  var timer = landingSpotlightTimers.get(root);
  if (timer) {
    window.clearInterval(timer);
    landingSpotlightTimers.delete(root);
  }
}

function setLandingSpotlightSlide(root, index) {
  var slides = root.querySelectorAll('.cwr-landing-spotlight-slide');
  var dots = root.querySelectorAll('.cwr-landing-spotlight-dot');
  ...
  root.dataset.spotlightIndex = String(nextIndex);
}

function startLandingSocialProofSpotlight(root) {
  stopLandingSocialProofSpotlight(root);
  if (!root || root.classList.contains('is-static')) return;

  var slides = root.querySelectorAll('.cwr-landing-spotlight-slide');
  if (slides.length < 2) return;

  var timer = window.setInterval(function () {
    if (root.dataset.spotlightPaused === '1') return;
    var current = parseInt(root.dataset.spotlightIndex || '0', 10);
    setLandingSpotlightSlide(root, current + 1);
  }, 6000);

  landingSpotlightTimers.set(root, timer);
}

function initLandingSocialProofSpotlight(homeArticle) {
  if (!homeArticle) homeArticle = document.querySelector('article.home');
  if (!homeArticle) return;

  var root = homeArticle.querySelector('[data-landing-spotlight]');
  if (!root) return;

  stopLandingSocialProofSpotlight(root);

  var items = LANDING_SOCIAL_PROOF_SPOTLIGHT;
  ...
  stage.className = 'cwr-landing-spotlight-stage';

  items.forEach(function (item, index) {
    var slide = document.createElement('article');
    slide.className = 'cwr-landing-spotlight-slide' + (index === 0 ? ' is-active' : '');
    slide.setAttribute('aria-roledescription', 'slide');
    slide.setAttribute('aria-label', (index + 1) + ' of ' + items.length + ': ' + item.name);

    slide.innerHTML =
      '<div class="cwr-landing-spotlight-logo">' +
      '<img src="' + escapeLandingSpotlightText(item.logo) + '" alt="' + escapeLandingSpotlightText(item.name) + ' logo" loading="' + (index === 0 ? 'eager' : 'lazy') + '">' +
      '</div>' +
      '<blockquote class="cwr-landing-spotlight-quote">' + escapeLandingSpotlightText(item.quote) + '</blockquote>' +
      '<p class="cwr-landing-spotlight-meta">' + escapeLandingSpotlightText(item.meta) + '</p>';

    stage.appendChild(slide);
  });

  root.appendChild(stage);
  root.dataset.spotlightIndex = '0';

  if (!staticMode) {
    var dots = document.createElement('div');
    dots.className = 'cwr-landing-spotlight-dots';
    dots.setAttribute('role', 'tablist');
    dots.setAttribute('aria-label', 'Choose featured client');

    items.forEach(function (item, index) {
      var dot = document.createElement('button');
      dot.type = 'button';
      dot.className = 'cwr-landing-spotlight-dot' + (index === 0 ? ' is-active' : '');
      dot.setAttribute('role', 'tab');
      dot.setAttribute('aria-label', item.name);
      dot.setAttribute('aria-selected', index === 0 ? 'true' : 'false');
      dot.setAttribute('tabindex', index === 0 ? '0' : '-1');
      dot.addEventListener('click', function () {
        setLandingSpotlightSlide(root, index);
        stopLandingSocialProofSpotlight(root);
        startLandingSocialProofSpotlight(root);
      });
    ...
    if (!root.dataset.spotlightBound) {
      root.dataset.spotlightBound = '1';
      root.addEventListener('mouseenter', function () {
        root.dataset.spotlightPaused = '1';
      });
      root.addEventListener('mouseleave', function () {
        root.dataset.spotlightPaused = '0';
      });
  ...
  root.dataset.spotlightReady = '1';
}

window.initLandingSocialProofSpotlight = initLandingSocialProofSpotlight;
window.stopLandingSocialProofSpotlight = stopLandingSocialProofSpotlight;
  ...
  for (let i = 0; i < pageArticles.length; i++) {
    if (pageName === pageArticles[i].dataset.page) {
      ...
      if (pageName === "home") {
        ...
        if (typeof window.initLandingSocialProofSpotlight === "function") {
          window.initLandingSocialProofSpotlight(document.querySelector('article.home'));
        }
        setTimeout(function () {
          if (typeof window.loadDynamicTestimonials === "function") {
            window.loadDynamicTestimonials();
        ...
        var spotlightRoot = document.querySelector('article.home [data-landing-spotlight]');
        if (spotlightRoot && typeof window.stopLandingSocialProofSpotlight === "function") {
          window.stopLandingSocialProofSpotlight(spotlightRoot);
        }
      }
...
(function() {
  ...
  window.prefillAdminClientEmail = function (data) {
    var els = adminClientEmailEls();
    if (!els.form) return;
  ...
  const BUSINESS_DOCS_RTD_DELETES_PATH = 'agencyBusinessDocDeletes';
  const BUSINESS_DOC_CLIENT_LOGOS_KEY = 'businessDocClientLogos.v1';
  /** While > 0, RTDB snapshot handler skips merging (avoids resurrecting mid-delete). */
  ...
  function loadBusinessDocs() {
    try {
      ...
      return parsed.filter(function (d) {
        return d && d.id && !isBusinessDocDeleted(d.id) && !isBusinessDocDeletedRemote(d.id);
      });
    } catch (e) {
  ...
  function saveBusinessDocs(docs) {
    try {
      var clean = (Array.isArray(docs) ? docs : []).filter(function (d) {
        return d && d.id && !isBusinessDocDeleted(d.id) && !isBusinessDocDeletedRemote(d.id);
      });
      localStorage.setItem(BUSINESS_DOCS_STORAGE_KEY, JSON.stringify(clean));
    } catch (e) {
  ...
  function isBusinessDocDeletedRemote(docId) {
    return !!businessDocRemoteDeletedIds[String(docId || '')];
  }

  function isBusinessDocRemoved(docId) {
    var id = String(docId || '');
    if (!id) return false;
    return isBusinessDocDeleted(id) || isBusinessDocDeletedRemote(id);
  }

  function applyRemoteBusinessDocDeletes(val) {
    businessDocRemoteDeletedIds = {};
    if (!val || typeof val !== 'object') return;
    Object.keys(val).forEach(function (id) {
      if (val[id]) businessDocRemoteDeletedIds[id] = true;
    });
  }

  function suppressBusinessDocsSync() {
    businessDocsSyncSuppressDepth += 1;
  }
  ...
  function pruneBusinessDocDeletedIds(remoteList, remoteDeleteVal) {
    var map = loadBusinessDocDeletedIds();
    var remoteIds = {};
    (remoteList || []).forEach(function (d) {
      if (d && d.id) remoteIds[d.id] = true;
    });
    var remoteDeletes = remoteDeleteVal && typeof remoteDeleteVal === 'object' ? remoteDeleteVal : {};
    var changed = false;
    Object.keys(map).forEach(function (id) {
      if (!remoteIds[id] && !remoteDeletes[id]) {
        delete map[id];
        changed = true;
      }
    });
  ...
  async function syncBusinessDocToRtdb(doc) {
    ...
    if (!clean || isBusinessDocRemoved(clean.id)) return;
  ...
  async function markBusinessDocDeletedRemote(docId) {
    if (!docId || !window.rtdb || !window.rtdbRef || !window.rtdbSet) return false;
    try {
      await window.rtdbSet(
        window.rtdbRef(window.rtdb, BUSINESS_DOCS_RTD_DELETES_PATH + '/' + docId),
        Date.now()
      );
      businessDocRemoteDeletedIds[String(docId)] = true;
      return true;
    } catch (e) {
      console.warn('Failed to mark business doc deleted in RTDB', e);
      return false;
    }
  }

  async function removeBusinessDocFromRtdb(docId) {
    if (!docId || !window.rtdb || !window.rtdbRef || !window.rtdbRemove) return false;
    await markBusinessDocDeletedRemote(docId);
    await window.rtdbRemove(window.rtdbRef(window.rtdb, BUSINESS_DOCS_RTD_PATH + '/' + docId));
    clearBusinessDocDeletedLocal(docId);
    return true;
  }
  ...
  function mergeBusinessDocLists(localList, remoteList) {
    ...
    (localList || []).forEach(function (d) {
      if (!d || !d.id || isBusinessDocRemoved(d.id)) return;
    ...
    return {
      docs: Object.keys(map)
        .filter(function (k) {
          return !isBusinessDocRemoved(k);
        })
        .map(function (k) {
          return map[k];
        }),
      toPush: toPush.filter(function (d) {
        return d && d.id && !isBusinessDocRemoved(d.id);
      })
    };
  }
  ...
  async function loadBusinessDocDeletesFromRtdb() {
    if (!window.rtdb || !window.rtdbRef || !window.rtdbGet) return null;
    try {
      var snap = await window.rtdbGet(window.rtdbRef(window.rtdb, BUSINESS_DOCS_RTD_DELETES_PATH));
      return snap.val();
    } catch (e) {
      console.warn('Failed to load business doc deletes from RTDB', e);
      return null;
    }
  }
  ...
  async function migrateBusinessDocsToRtdbIfNeeded() {
    ...
    var deletesVal = await loadBusinessDocDeletesFromRtdb();
    if (deletesVal) applyRemoteBusinessDocDeletes(deletesVal);
    ...
    if (!fromRtdb.length) {
      ...
      for (var i = 0; i < businessDocs.length; i++) {
        if (!isBusinessDocRemoved(businessDocs[i].id)) {
    ...
    pruneBusinessDocDeletedIds(fromRtdb, deletesVal);
  ...
  var businessDocDeletesUnsub = null;

  function handleBusinessDocsRtdbSnapshot(docsVal, deletesVal) {
    if (businessDocsSyncSuppressDepth > 0) return;
    applyRemoteBusinessDocDeletes(deletesVal);
    if (!docsVal || typeof docsVal !== 'object') {
      docsVal = {};
    }
    var remote = Object.keys(docsVal).map(function (k) {
      return normalizeBusinessDocRecord(docsVal[k], k);
    });
  ...
  function subscribeBusinessDocsFromRtdb() {
    ...
    businessDocDeletesUnsub = window.rtdbOnValue(
      window.rtdbRef(window.rtdb, BUSINESS_DOCS_RTD_DELETES_PATH),
      function (snap) {
        deletesValCache = snap.val();
        emitMerged();
      },
      function (err) {
        console.warn('Business doc deletes RTDB listen failed', err);
      }
    );
  }
  ...
  function unsubscribeBusinessDocsFromRtdb() {
    ...
    if (typeof businessDocDeletesUnsub === 'function') {
      try {
        businessDocDeletesUnsub();
      } catch (e) {}
    }
  ...
  /** Escape for list/card HTML — local so render never depends on later helpers. */
  function escapeBusinessDocListHtml(str) {
    if (str == null || str === '') return '';
    return String(str)
      .replace(/&/g, '&amp;')
      .replace(/</g, '&lt;')
  ...
  function buildBusinessDocItemEl(doc) {
    ...
    typeRow.innerHTML =
      ...
      escapeBusinessDocListHtml(doc.type) +
      '">' +
      escapeBusinessDocListHtml(typeBadge) +
      '</span>';
    ...
    statusWrap.innerHTML =
      ...
      escapeBusinessDocListHtml(doc.status) +
      '">' +
      escapeBusinessDocListHtml(String(doc.status || '').toUpperCase()) +
      '</span>';
  ...
  function ensureBusinessDocsListRoot() {
    ...
    if (list) {
      var parentWrap = list.closest ? list.closest('.business-docs-table-wrapper') : null;
      if (parentWrap) parentWrap.classList.remove('has-scrollbar');
      return list;
    }
    ...
    return document.getElementById('business-docs-list') || wrap.firstElementChild;
  }

  function renderBusinessDocsFlatFallback(listRoot, docs) {
    listRoot.textContent = '';
    listRoot.className = 'business-docs-client-list';
    docs.forEach(function (doc) {
      try {
        listRoot.appendChild(buildBusinessDocItemEl(doc));
      } catch (itemErr) {
        var row = document.createElement('div');
        row.className = 'business-docs-doc-item';
        row.textContent =
          String((doc && doc.clientName) || 'No client') +
          ' · ' +
          String((doc && doc.type) || 'document') +
          ' · ' +
          String((doc && doc.status) || '');
  ...
  function renderBusinessDocs() {
    ...
    var filtered = applyBusinessDocsFilters(
      businessDocs.slice().sort(function (a, b) {
        return new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime();
      })
    );
    ...
    if (!listRoot) {
      // Last-resort: keep whatever table markup exists — never wipe rows when docs exist.
    ...
    try {
      if (filtered.length === 0) {
        ...
        if (
          businessDocsOpenClientKey &&
          !groups.some(function (g) {
            return g.key === businessDocsOpenClientKey;
          })
        ) {
      ...
      console.error('renderBusinessDocs failed', err);
      try {
        renderBusinessDocsFlatFallback(listRoot, filtered);
      } catch (fallbackErr) {
        console.error('renderBusinessDocs fallback failed', fallbackErr);
        listRoot.textContent = 'Could not render documents. Check the console for details.';
      }
    }
  ...
  async function handleDeleteDocumentConfirmClick() {
    ...
    renderBusinessDocs();
    closeDeleteDocumentConfirmModal();
    var cloudRemoved = false;
    var authError = null;
    try {
      if (typeof window.waitForAdminAuth === 'function') {
        await window.waitForAdminAuth(8000);
      }
      cloudRemoved = await removeBusinessDocFromRtdb(docId);
    } catch (err) {
      authError = err;
      console.warn('Failed to remove business doc from RTDB', err);
    } finally {
      releaseBusinessDocsSync();
    }
    if (!cloudRemoved && authError && typeof window.alert === 'function') {
      window.alert(
        'Removed from this browser. Cloud delete failed — stay signed in with Google (ruben.jim.co@gmail.com) and try again.'
      );
    }
    if (
      businessDocIdInput &&
  ...
  (function initAdminMobileTabBarOrder() {
    ...
    function captureTabHomes() {
      ...
      if (hub) {
        tabHomes['content-hub'] = { parent: hub.parentNode, next: hub.nextSibling };
      }
    }
    ...
    function mountReorderStrip() {
      ...
      currentOrder.forEach(function (id) {
        var btn =
          id === 'content-hub'
            ? document.getElementById('admin-tab-content-hub')
            : tabBar.querySelector('.admin-tab[data-admin-tab="' + id + '"]');
    ...
    function unmountReorderStrip() {
      ...
      currentOrder.forEach(function (id) {
        var btn =
          id === 'content-hub'
            ? reorderStrip.querySelector('#admin-tab-content-hub')
            : reorderStrip.querySelector('.admin-tab[data-admin-tab="' + id + '"]');
  ...
  (function initAdminMobileContentSubtabBar() {
    ...
    var subtabRoot = null;
    var subtabBar = null;
    var contentHub = document.getElementById('admin-tab-content-hub');
    ...
    function ensureSubtabBar() {
      if (subtabRoot) return subtabRoot;
      subtabRoot = document.createElement('div');
      subtabRoot.id = 'admin-mobile-subtab-bar-root';
      subtabRoot.className = 'admin-mobile-subtab-bar-root';
      subtabRoot.hidden = true;

      subtabBar = document.createElement('nav');
      subtabBar.className = 'admin-mobile-subtab-bar';
      subtabBar.setAttribute('aria-label', 'Content sections');

      CONTENT_SUB_TAB_IDS.forEach(function (tabId) {
        var meta = CONTENT_SUB_META[tabId];
        var btn = document.createElement('button');
        btn.type = 'button';
      ...
      subtabRoot.appendChild(subtabBar);
      document.body.appendChild(subtabRoot);
      return subtabRoot;
    }

    function syncSubtabBar(activeTabId) {
      ensureSubtabBar();
      var show =
        isMobileBar() &&
        isSignedInAdmin() &&
      ...
      if (contentHub) {
        contentHub.classList.toggle('is-active', show);
        contentHub.setAttribute('aria-selected', show ? 'true' : 'false');
      }

      if (!subtabBar) return;
      subtabBar.querySelectorAll('.admin-mobile-subtab').forEach(function (btn) {
    ...
    if (contentHub) {
      contentHub.addEventListener('click', function () {
        if (window.AdminMobileTabOrder && window.AdminMobileTabOrder.isReorderActive()) return;
        var last = 'portfolio';
        try {
          last = sessionStorage.getItem(LAST_CONTENT_KEY) || 'portfolio';
        } catch (e) {}
        if (CONTENT_SUB_TAB_IDS.indexOf(last) < 0) last = 'portfolio';
        if (typeof window.adminActivateTab === 'function') window.adminActivateTab(last);
      });
    }

    window.syncAdminMobileContentSubtabBar = syncSubtabBar;
    window.saveAdminLastContentTab = function (tabId) {
      if (CONTENT_SUB_TAB_IDS.indexOf(tabId) < 0) return;
      try {
        sessionStorage.setItem(LAST_CONTENT_KEY, tabId);
      } catch (e) {}
    };
    ...
    window.addEventListener('resize', function () {
      ...
      syncSubtabBar(active ? active.getAttribute('data-admin-tab') : 'overview');
    });
  })();
...
function killEaseInAnimations() {
  easeInObservers.forEach(function (observer) {
    observer.disconnect();
  });
  easeInObservers = [];

  document.querySelectorAll('.cwr-ease-in').forEach(function (el) {
    el.classList.remove('cwr-ease-in', 'is-visible');
    el.style.removeProperty('transition-delay');
  });
}

function prepareEaseInElements(section, itemSelector, staggerMs) {
  if (!section) return [];
  var items = section.querySelectorAll(itemSelector);
  var delayStep = staggerMs != null ? staggerMs : 100;
  items.forEach(function (el, index) {
    el.classList.add('cwr-ease-in');
    el.style.transitionDelay = (index * delayStep) + 'ms';
  });
  return items;
}

function revealEaseInElements(section) {
  if (!section) return;
  section.querySelectorAll('.cwr-ease-in').forEach(function (el) {
    el.classList.add('is-visible');
  });
}

function observeEaseInSection(section) {
  if (!section || prefersReducedMotion()) return;

  if (typeof IntersectionObserver === 'undefined') {
    revealEaseInElements(section);
    return;
  }
  ...
  var observer = new IntersectionObserver(
    function (entries) {
      entries.forEach(function (entry) {
        ...
        revealEaseInElements(entry.target);
...
function initLandingOfferTypewriter(homeArticle) {
  ...
  var nodes = offer.querySelectorAll('[data-cwr-type-text]');
  if (!nodes.length) return;

  window.__cwrLandingOfferTyped = true;

  var plus = offer.querySelector('.cwr-landing-offer-plus--delayed');

  function showFinalText() {
    nodes.forEach(function (node) {
      node.textContent = node.getAttribute('data-cwr-type-text') || '';
      node.classList.remove('is-typing');
      node.classList.add('is-typed');
    });
    if (plus) plus.classList.add('is-visible');
    offer.classList.add('is-typed-complete');
  }
  ...
  nodes.forEach(function (node) {
    node.textContent = '';
    node.classList.remove('is-typed', 'is-typing');
  });
  if (plus) plus.classList.remove('is-visible');
  offer.classList.remove('is-typed-complete');

  var charDelay = 68;

  function typeWord(node, done) {
    var text = node.getAttribute('data-cwr-type-text') || '';
    var index = 0;
    ...
    function step() {
      ...
      node.classList.remove('is-typing');
      node.classList.add('is-typed');
      if (typeof done === 'function') window.setTimeout(done, 180);
    }
  ...
  window.setTimeout(function () {
    typeWord(nodes[0], function () {
      if (plus) plus.classList.add('is-visible');
      if (nodes[1]) {
        window.setTimeout(function () {
          typeWord(nodes[1], function () {
            offer.classList.add('is-typed-complete');
          });
        }, 120);
      } else {
...
window.initLandingOfferTypewriter = initLandingOfferTypewriter;
  ...
  if (copy) {
    ...
    requestAnimationFrame(function () {
      requestAnimationFrame(function () {
        ...
        if (typeof initLandingOfferTypewriter === 'function') {
...
function initHomeLandingEaseIn(homeArticle) {
  var bands = [
    {
    ...
    },
    {
      ...
      items: '.cwr-landing-band-header, .cwr-landing-spotlight',
      stagger: 100
    },
    {
  ...
  bands.forEach(function (band) {
    if (!band.el) return;
    prepareEaseInElements(band.el, band.items, band.stagger);
    observeEaseInSection(band.el);
  });
}

function initAboutEaseIn(aboutArticle) {
  var sections = [
    { el: aboutArticle.querySelector('section.home-hero'), items: '.home-hero-inner, .home-hero-visual', stagger: 110 },
    { el: aboutArticle.querySelector('section.bento-grid'), items: '[data-bento-cell]', stagger: 100 },
    { el: aboutArticle.querySelector('section.home-who-i-am'), items: '.home-who-i-am-card, .work-marquee', stagger: 110 },
    { el: aboutArticle.querySelector('section.testimonials'), items: '.testimonials-item', stagger: 80 },
    { el: aboutArticle.querySelector('section.clients'), items: '.clients-title, .clients-scroll-shell', stagger: 100 }
  ];

  sections.forEach(function (block) {
    if (!block.el) return;
    prepareEaseInElements(block.el, block.items, block.stagger);
    observeEaseInSection(block.el);
  });
}

window.initActivePageScrollReveals = function () {
  requestAnimationFrame(function () {
    killEaseInAnimations();
    ...
    if (page === 'home') {
      if (!prefersReducedMotion()) {
        initHomeHeroEaseIn(activeArticle);
        initHomeLandingEaseIn(activeArticle);
      } else if (typeof initLandingOfferTypewriter === 'function') {
        initLandingOfferTypewriter(activeArticle);
      }
    } else if (page === 'about') {
      if (!prefersReducedMotion()) {
        initAboutEaseIn(activeArticle);
      }
    }
  });
};

var easeInResizeTimer;
window.addEventListener('resize', function () {
  clearTimeout(easeInResizeTimer);
  easeInResizeTimer = setTimeout(function () {
    if (typeof window.initActivePageScrollReveals === 'function') {
      window.initActivePageScrollReveals();
    }
  }, 250);
